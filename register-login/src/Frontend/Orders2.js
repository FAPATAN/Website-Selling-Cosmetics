...
export default function Orders2() {
    ...
    // countdown timer (?????? order ????????????????: Status='O' ???? 'P' ??????????????? Invoice_pic)
    const needsPayment = order?.Status === 'O' || (order?.Status === 'P' && !order?.Invoice_pic);

    // ??????????????? render ? ????????? useEffect cycle ???
    // ??? Order_date ??????????? (data error) ?????? Date.now() ???? effective start ???
    ...
    useEffect(() => {
        ...
        const tick = () => {
            ...
            if (diff <= 0) {
                ...
                setExpired(true);
                setOrder(prev => prev ? { ...prev, Status: 'Ca' } : prev); // ?????? UI ????? ????? API
    ...
    const handleAddressSave = async () => {
        ...
        try {
            ...
            if (!res.ok) { setAddressError(data.error || '??????????????'); return; }
    ...
    // O ???? P ???????? slip = Order List, P ????? slip = To Pay, A/S/R/C/Ca = ???????
    const currentStatus = (() => {
            ...
            {showCancelModal && (
                <div className="cancel-modal-backdrop" onClick={() => setShowCancelModal(false)}>
                    <div className="cancel-modal" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="cancel-modal-title">????????????????</div>
                        <div className="cancel-modal-desc">????????????????????????????????????????<br/>???????????????????????????????????</div>
                        <div className="cancel-modal-actions">
                            <button className="cancel-modal-no" onClick={() => setShowCancelModal(false)}>?????????</button>
                            <button className="cancel-modal-yes" onClick={confirmCancel}>????????????</button>
                        </div>
                    </div>
                </div>
            )}
                            ...
                            <div className="order-status-tracker" style={{ marginTop: 0 }}>
                                ...
                                {order.TrackingNo && (
                                    <div style={{ padding: '14px 20px 0' }}>
                                        <div style={{ background: '#ffffff', border: '1px solid #FF8FAB', borderRadius: 10, padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 10 }}>
                                            ...
                                            <div>
                                                <div style={{ fontSize: 14, fontWeight: 700, color: '#000000', textTransform: 'uppercase', letterSpacing: '.5px', marginBottom: 3 }}>???????????????</div>
                                    ...
                                    <div style={{ padding: '10px 20px 0' }}>
                                        <div style={{ background: '#fafafa', border: '1px solid #eee', borderRadius: 10, padding: '12px 16px' }}>
                                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                                                <div style={{ fontSize: 15, fontWeight: 700, color: '#000000', textTransform: 'uppercase', letterSpacing: '.8px' }}>?????????????</div>
                                            ...
                                            {!showAddressModal ? (
                                                ...
                                                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                                                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px 14px' }}>
                                                        <div>
                                                            <div style={{ fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '.5px', color: '#9a9a9a', marginBottom: 4 }}>????</div>
                                                            <input
                                                    ...
                                                    <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, marginTop: 2 }}>
                                                        <button onClick={cancelAddressEdit} style={{ padding: '7px 16px', borderRadius: 8, border: '1px solid rgba(0,0,0,0.08)', background: 'none', fontSize: 12.5, fontWeight: 600, cursor: 'pointer', color: '#6b6b6b' }}>??????</button>
                                ...
                                <div className="pending-order-footer">
                                    ...
                                    {(needsPayment && !isExpired)
                                            ? ' ???????????????????'
                                            : { P: ' ?????????', A: ' ????????????????', S: ' ????????????????', R: ' ?????????????', C: ' ??????????????', Ca: ' ????????????????'}[isExpired ? 'Ca' : order.Status] || ''
                                        }
                                    </div>
                                    {(needsPayment || isExpired) && (
                                            ...
                                            {(isExpired || expired) ? (
                                                ...
                                                <>
                                                    ...
                                                    <div style={{ display: "flex", gap: "10px", alignItems: "center" }}>
                                                        ...
                                                        <button className="pending-order-cancel-btn" onClick={handleCancel}>????????????</button>
                                                    </div>
                                                </>
                                            )}
                                        </div>
