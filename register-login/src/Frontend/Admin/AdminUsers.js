  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!res.ok || data.error) {
        alert(`บันทึกไม่สำเร็จ: ${data.error || res.status}`);
        setSaving(false);
        return;
      }
    } catch (err) {
      ...
      <div className="su-app">
        ...
        <aside className="su-sidebar">
          ...
          {navItems.map((item) => (
            ...
            >
              {item.iconSrc ? <img src={item.iconSrc} alt={item.label} className="su-nav-icon" /> : <span style={{width:'20px',height:'20px',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'14px',flexShrink:0}}>🏷️</span>}
              {item.label}
            </button>
          ))}
          <div style={{ flex: 1 }} />
