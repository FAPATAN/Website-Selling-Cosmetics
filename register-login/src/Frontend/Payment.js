      ...
      <div className="pay-page">
        ...
        <div className="pay-left">
          ...
          <nav className="ship-breadcrumb">
            ...
            <span className="bc-sep">&gt;</span>
            <span style={{ cursor: "pointer" }} onClick={() => navigate("/shipping")}>Shipping</span>
            <span className="bc-sep">&gt;</span>
            <span className="bc-active">Payment</span>
          </nav>

          {/* Contact / Ship to / Shipping method summary */}
          <div className="ship-summary-box">
            ...
            <div className="ship-summary-divider" />
            <div className="ship-summary-row">
              <span className="ship-summary-label">Shipping method</span>
              <span className="ship-summary-value">Flash Express · FREE</span>
            </div>
          </div>
          ...
          <div className="pay-section">
            <div className="pay-section-header">
              <h3 className="pay-section-title">Payment</h3>
              <p className="pay-section-sub">All transactions are secure and encrypted.</p>
            </div>

          
            <div className={`pay-option${paymentOption === "bank" ? " selected" : ""}`} onClick={() => setPaymentOption("bank")}>
              <div className="pay-radio-wrap">
                <div className={`pay-radio${paymentOption === "bank" ? " active" : ""}`}>
                  {paymentOption === "bank" && <span className="pay-radio-dot" />}
                </div>
              </div>
              <div className="pay-bank-info">
                <span className="pay-bank-name">บัญชี ธ.กสิกรไทย</span>
                <span className="pay-bank-sub">xxx-x-x9130-x · บจก. แอดเอ็นดับเบิ้ลยู</span>
              </div>
              <img src="/bank-logo.jpg" alt="bank" className="pay-bank-logo" />
            </div>
          </div>

          {/* Billing address */}
          <div className="pay-section pay-billing-section">
            <h3 className="pay-section-title">Billing address</h3>
            <p className="pay-section-sub">Select the address that matches your card or payment method.</p>

            <div
              className={`pay-option${billingOption === "same" ? " selected" : ""}`}
              onClick={() => setBillingOption("same")}
            >
              <div className="pay-radio-wrap">
                <div className={`pay-radio${billingOption === "same" ? " active" : ""}`}>
                  {billingOption === "same" && <span className="pay-radio-dot" />}
                </div>
              </div>
              <span className="pay-option-label">Same as shipping address</span>
            </div>

            <div
              className={`pay-option${billingOption === "different" ? " selected" : ""}`}
              onClick={() => setBillingOption("different")}
            >
              <div className="pay-radio-wrap">
                <div className={`pay-radio${billingOption === "different" ? " active" : ""}`}>
                  {billingOption === "different" && <span className="pay-radio-dot" />}
                </div>
              </div>
              <span className="pay-option-label">Use a different billing address</span>
            </div>
          </div>

          {/* Actions */}
          <div className="ship-actions">
            <button className="ship-back-btn" onClick={() => navigate("/shipping")}>
              &larr; Return to shipping
            </button>
            <button className="pay-now-btn" onClick={() => navigate("/pay")}>
              Pay now
            </button>
          </div>
