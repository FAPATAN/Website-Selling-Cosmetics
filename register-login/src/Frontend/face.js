...
const FaceSection = ({ setIsRegisterView }) => {
	...
	useEffect(() => {
		const fetchAllProducts = async () => {
			try {
				const res = await fetch(`${API}/api/face`);
				const result = await res.json();
				setAllProducts(Array.isArray(result.data) ? result.data : []);
				// ปรับให้ face เป็น 4 ตัวแรกของสินค้าทั้งหมด (รองรับชื่อไฟล์ face_1.1.jpg, face_1.2.jpg ฯลฯ)
				setFace((Array.isArray(result.data) ? result.data : []).slice(0, 4));
			} catch (err) {
				setAllProducts([]);
				setFace([]);
			}
	...
	// helper: match image path ทั้ง "face_1.1.jpg" และ "products/face_1.1.jpg"
	...
	return (
		<>
      	...
      	<section className="product-categories-section">
			...
			{/* FACE Section Header (no plus icon) */}

			<section className="section-header-face">
			...
			<div className="face-content-layout">
...
{/* แถวสินค้า dynamic: 4 สินค้าต่อแถว เพิ่มได้เรื่อยๆ */}
							<div className="face-grid" style={{ margin: '0 0 64px 0' }}>
								{(() => {
									const REGEX = /^face_(\d+)\./i;
									...
									const groupedCards = groupNums.map(num => {
										const prefix = `face_${num}`;
										const variantKey = selectedImages[num] || `face_${num}.1`;
										...
										return (
											<div className="face-card" key={`g_${num}`}>
                                ...
                                {product.Product_model && <div className="card-model">{product.Product_model}</div>}
									...
									const standaloneCards = standaloneProducts.map(product => {
										...
										return (
											<div className="face-card" key={`s_${product.Product_id}`}>
												<a href={`/best1/${product.Product_id}`} style={{textDecoration:'none'}}>
													<div style={{position:'relative', display:'block'}}>
													<div className="card-image" style={{
														backgroundImage: `url(${imgSrc})`,
