            ...
            <div className="account-layout">
                ...
                <div className="account-content">
                    <div className="orders-header">
                        <span className="orders-title">การซื้อของฉัน</span>
                    </div>
                    ...
                    <div className="orders-list">
                        {loadingOrders ? (
                            ...
                            <div style={{ padding: "32px", textAlign: "center", color: "#999" }}>ไม่มีรายการสั่งซื้อ</div>
                        ) : (
                            <div className="order-numbered-list">
                                {pendingOrders.map((order, idx) => {
                                    ...
                                    const statusBadge = {
                                        O:  { cls: "order-created",         text: " สร้างรายการสั่งซื้อ" },
                                        P:  { cls: "waiting",               text: " รอตรวจสอบ" },
                                        A:  { cls: "slip-sent",             text: " ยืนยันคำสั่งซื้อ" },
                                        S:  { cls: "status-delivered",      text: " จัดส่งสินค้าแล้ว" },
                                        R:  { cls: "status-delivered",      text: " ถึงผู้รับแล้ว" },
                                        C:  { cls: "slip-sent",             text: " เสร็จเรียบร้อย" },
                                        Ca: { cls: "status-cancelled-order",text: " ยกเลิกคำสั่งซื้อ" },
                                    }[displayStatus] || { cls: "order-created", text: " สร้างรายการสั่งซื้อ" };
                                    return (
                                        <div
                                            key={order.Order_id}
                                                ...
                                                <div className="order-numbered-left">
                                                    ...
                                                    <div className="order-numbered-info">
                                                        <span className="order-numbered-title">
                                                            {items[0]?.name || `Order #${order.Order_id}`}{items.length > 1 ? ` +${items.length - 1} รายการ` : ""}
                                                        </span>
            ...
            {showLoginModal && (
                <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.45)',zIndex:9999,display:'flex',alignItems:'center',justifyContent:'center'}} onClick={() => setShowLoginModal(false)}>
                    <div style={{background:'#fff',borderRadius:16,padding:'36px 40px',maxWidth:340,width:'90%',textAlign:'center',boxShadow:'0 8px 32px rgba(0,0,0,0.18)'}} onClick={e => e.stopPropagation()}>
                        ...
                        <div style={{color:'#888',fontSize:14,marginBottom:24}}>คุณต้อง login เพื่อดูประวัติการสั่งซื้อ</div>
                        <div style={{display:'flex',gap:10,justifyContent:'center'}}>
                            <button onClick={() => navigate(-1)} style={{padding:'10px 22px',borderRadius:8,border:'1px solid #ddd',background:'#f5f5f5',cursor:'pointer',fontWeight:600}}>ยกเลิก</button>
