...
export default function Pay() {
  ...
  const [dbOrderId, setDbOrderId] = useState(null); // Order_id ที่ได้จาก DB
  ...
  const handleSlipSubmit = async () => {
    ...
    // 1. อัปเดต localStorage (คงไว้เพื่อความเข้ากันได้กับ Orders page)
    ...
    try {
      ...
      if (orderId) {
        ...
        if (!result.success) {
          ...
          // บันทึก dbStatus = 'P' ลง localStorage (ยังรอตรวจสอบ)
  ...
  useEffect(() => {
    ...
    if (found?.dbOrderId) {
      ...
      // มาจาก Orders2 โดยตรง — targetOrderId คือ DB Order_id อยู่แล้ว
    ...
    fetch(`${API}/api/cart/${memberId}`)
      ...
      .then((data) => {
        if (data.cart) {
          ...
          setCart(items);
          setCartCount(0); // ล้าง cart badge ใน header เมื่อมาถึงหน้า Pay (ข้อมูลยังอยู่ใน backend)
          ...
          // บันทึก pending order ลง localStorage เมื่อ user มาหน้า Pay
          if (items.length > 0) {
            const existing = JSON.parse(localStorage.getItem("pendingOrders") || "[]");
            const activeOrders = existing.filter(
              (o) => o.memberId === memberId && o.deadline > Date.now()
            );
            // เช็คว่ามี order ที่มีสินค้าชุดเดียวกันอยู่แล้วหรือเปล่า (กันสร้างซ้ำเมื่อ refresh)
            ...
            if (!alreadySameCart) {
              ...
              fetch(`${API}/api/orders/create`, {
                ...
                .then(dbData => {
                  ...
                  setDbOrderId(dbOId);
                  // บันทึก dbOrderId ลง localStorage
              ...
              // order นี้สร้างไปแล้ว ดึง dbOrderId จาก localStorage
