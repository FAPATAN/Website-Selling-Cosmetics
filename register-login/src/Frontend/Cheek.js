...
const CheekSection = ({ setIsRegisterView }) => {
	const navigate = useNavigate();
	const [categories, setCategories] = useState([]);
	const [cheekProducts, setCheekProducts] = useState([]);
	...
	// Fetch ALL products from backend (cheek)
	useEffect(() => {
		const fetchAllProducts = async () => {
			try {
				const res = await fetch("http://localhost:5000/api/cheek");
	...
	useEffect(() => {
		fetch("http://localhost:5000/api/price-range/7")
			.then(res => res.json())
			.then(({ min, max }) => {
				setMinPrice(min);
				setMaxPrice(max);
	...
	// helper: match image path ทั้ง "cheek_1.1.jpg" และ "products/cheek_1.1.jpg"
	...
	return (
		<>
      	...
      	<section className="product-categories-section">
			...
			{/* CHEEK Section Header (no plus icon) */}

			<section className="section-header-cheek">
				<div className="header-container">
					<h2 className="section-title">CHEEK</h2>
				</div>
			</section>

			{/* Filter Sidebar (Availability, Price) */}
			<div className="cheek-content-layout">
							...
							<div className="cheek-grid" style={{ margin: '0 0 64px 0' }}>
									{(() => {
										...
										const groupedCards = groupNums.map(num => {
											const prefix = `cheek_${num}`;
											const variantKey = selectedImages[num] || `cheek_${num}.1`;
											...
											return (
												<div className="cheek-card" key={`g_${num}`}>
													<a href={`/best1/${product.Product_id}`} style={{textDecoration:'none', position:'relative', display:'block'}}>
														...
														{product.Stock != null && Number(product.Stock) === 0 && (
															<div style={{position:'absolute',top:'50%',left:'50%',
																transform:'translate(-50%,-50%)',width:'86px',height:'86px',
                                ...
                                {product.Product_model && <div className="card-model">{product.Product_model}</div>}
										...
										const standaloneCards = standaloneProducts.map(product => {
											...
											return (
												<div className="cheek-card" key={`s_${product.Product_id}`}>
													<a href={`/best1/${product.Product_id}`} style={{textDecoration:'none', position:'relative', display:'block'}}>
														<div className="card-image" style={{
															backgroundImage: `url(${imgSrc})`,
															width: '180px', height: '180px',
