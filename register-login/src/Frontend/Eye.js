...
const EyeSection = ({ setIsRegisterView }) => {
	...
	useEffect(() => {
		const fetchAllProducts = async () => {
			try {
				...
				// ??????? eye ???? 4 ?????????????????????? (?????????????? eye_1.1.jpg, eye_1.2.jpg ???)
				setEye((Array.isArray(result.data) ? result.data : []).slice(0, 4));
			} catch (err) {
				setAllProducts([]);
				setEye([]);
			}
	...
	// helper: match image path ???? "eye_1.1.jpg" ??? "products/eye_1.1.jpg"
	...
	return (
		<>
      	...
      	<section className="product-categories-section">
			...
			{/* EYE Section Header (no plus icon) */}

			<section className="section-header-Eye">
				<div className="header-container">
					<h2 className="section-title">EYE</h2>
				</div>
			</section>

			{/* Filter Sidebar (Availability, Price) */}
			<div className="Eye-content-layout">
...
{/* ????????? dynamic: 4 ???????????? ??????????????? */}
							<div className="Eye-grid" style={{ margin: '0 0 64px 0' }}>
								{(() => {
									const REGEX = /^eye_(\d+)\./i;

									// Products following eye_N.M naming convention ? grouped with color variants
									...
									// Products NOT following convention ? show as standalone cards
									...
									const groupedCards = groupNums.map(num => {
										...
										return (
											<div className="Eye-card" key={`g_${num}`}>
                                ...
                                {product.Product_model && <div className="card-model">{product.Product_model}</div>}
									...
									const standaloneCards = standaloneProducts.map(product => {
										...
										return (
											<div className="Eye-card" key={`s_${product.Product_id}`}>
												<a href={`/best1/${product.Product_id}`} style={{textDecoration:'none', position:'relative', display:'block'}}>
													<div className="card-image" style={{
														backgroundImage: `url(${imgSrc})`,
														width: '180px', height: '180px',
